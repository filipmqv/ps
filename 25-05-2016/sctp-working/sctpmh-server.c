...
int main(int argc, char **argv) {
  ...
  for(i = 0; i < no; i++)
    printf("ADDR %d: %s:%d\n", i + 1, inet_ntoa((*laddrs)[i].sin_addr), ntohs((*laddrs)[i].sin_port));
  ...
  while (1) {
    ...
    now = time(NULL);
    snprintf(buf, 1024, " my local %s", ctime(&now));
    sctp_sendmsg(sfd, (void*) buf, (size_t) strlen(buf) + 1, (struct sockaddr*) &raddr, sl, 0, 0,
                 LOCAL_TIME_STREAM, 0, 0);
    snprintf(buf, 1024, " my green %s", asctime(gmtime(&now)));
    sctp_sendmsg(sfd, (void*) buf, (size_t) strlen(buf) + 1, (struct sockaddr*) &raddr, sl, 0, 0,
